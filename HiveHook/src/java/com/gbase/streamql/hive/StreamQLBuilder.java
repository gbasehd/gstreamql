    ...
    public String getSql() throws Exception {
        ...
        switch (parser.getCmdType()) {
            case CREATE_STREAMJOB: {
                sql = "Insert into " +
                       Conf.SYS.DB + ".streamjobmgr(name, pid, jobid, status, define) values ('" +
                       this.parser.getStreamJobName() + "',NULL,NULL,'STOPPED','" +
                       this.parser.getStreamJobDef() + "')";
                break;
            }
            case SHOW_STREAMJOBS: {
                sql = "Select name, jobid, status, define from " + Conf.SYS.DB + ".streamjobmgr";
                break;
            }
            case START_STREAMJOB: {
                sql = "Update " +
                       Conf.SYS.DB +".streamjobmgr set status = '" + STATUS.RUNNING.toString() +
                       "' , pid = \"" + this.job.getPid() +
            ...
            case STOP_STREAMJOB: {
                sql = "Update " +
                       Conf.SYS.DB +".streamjobmgr set status = '" + STATUS.STOPPED.toString() +
                       "' , pid = \"NULL\", jobid = \"NULL\" where  name = \"" + this.parser.getStreamJobName() + "\"";
                break;
            }
            case DROP_STREAMJOB: {
                sql = "Delete from " +
                       Conf.SYS.DB +".streamjobmgr where  name = \"" + this.parser.getStreamJobName() + "\"";
                break;
            }
            case UNMATCHED:
                break;
